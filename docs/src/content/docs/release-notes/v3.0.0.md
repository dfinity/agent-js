...
| Package                       | Before    | After    | Approx. Size Delta |
| ----------------------------- | --------- | -------- | ------------------ |
| `@dfinity/identity-secp256k1` | 106.69 kB | 32.95 kB | - 69% 🔽           |
| `@dfinity/identity`           | 47.8 kB   | 18.76 kB | - 61% 🔽           |
| `@dfinity/auth-client`        | 50.15 kB  | 19.4 kB  | - 61% 🔽           |
| `@dfinity/agent`              | 72.1 kB   | 47.45 kB | - 34% 🔽           |
| `@dfinity/assets`             | 67.97 kB  | 52.51 kB | - 22% 🔽           |
| `@dfinity/principal`          | 4.21 kB   | 4.44 kB  | + 5% 🔺            |
| `@dfinity/candid`             | 11.82 kB  | 12.96 kB | + 10% 🔺           |

### Clock Drift Calculation

You can make the `HttpAgent` account for clock drifts between the local clock and the IC network, when calculating the calls' ingress expiry. This is done by syncing the local clock with the IC network. Use the `shouldSyncTime` option to enable this behavior.
...
The agent no longer checks for watermarks on query responses. It now checks if the node signature is not older than the `ingressExpiryInMinutes` option, to prevent stale query responses. This takes into account [clock drift](#clock-drift-calculation), if enabled.
