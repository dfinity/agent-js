...
The management canister has been moved to its own package, `@dfinity/ic-management`. You will need to update your code to use this new package for management canister interactions.
...
```typescript
import { getManagementCanister } from '@dfinity/ic-management';

const management = getManagementCanister({ agent });
```

You will also need to add `@dfinity/ic-management` as a dependency to your project.
...
The `fromHex`, `toHex`, and `concat` utility functions have been replaced with `bytesToHex`, `hexToBytes`, and `concatBytes` from `@noble/hashes/utils`.
...
The `hash` function has been replaced with `sha256` from `@noble/hashes/sha2`.
...
The `@dfinity/agent` package now uses [`@dfinity/cbor`](https://npmjs.com/package/@dfinity/cbor) instead of the `borc` and `simple-cbor` dependencies, reducing [a lot](https://github.com/dfinity/agent-js/pull/1015#issuecomment-2912381841) the bundle size.
...
- **Watermark Check for Query Responses**: The agent now checks if the node signature is not older than the `ingressExpiryInMinutes` option, taking into account clock drift, to prevent stale query responses.
