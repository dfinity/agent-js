...
var transforms_js_2 = require('./transforms.js');
Object.defineProperty(exports, 'makeNonce', {
  enumerable: true,
  get: function () {
    return transforms_js_2.makeNonce;
  },
});
...
(function (RequestStatusResponseStatus) {
  ...
  (RequestStatusResponseStatus =
    exports.RequestStatusResponseStatus || (exports.RequestStatusResponseStatus = {})),
);
...
class HttpDefaultFetchError extends errors_js_1.AgentError {
  constructor(message) {
    super(message);
    this.message = message;
  }
}
class IdentityInvalidError extends errors_js_1.AgentError {
  constructor(message) {
    super(message);
    this.message = message;
  }
...
class HttpAgent {
  constructor(options = {}) {
    this.rootKey = (0, buffer_js_1.fromHex)(exports.IC_ROOT_KEY);
    this._pipeline = [];
    ...
    this._identity = Promise.resolve(options.identity || new auth_js_1.AnonymousIdentity());
    // Add a nonce transform to ensure calls are unique
    if (!options.disableNonce) {
      this.addTransform((0, transforms_js_1.makeNonceTransform)(transforms_js_1.makeNonce));
    }
  }
  isLocal() {
  ...
  async getPrincipal() {
    ...
    return principal_1.Principal.from((await this._identity).getPrincipal());
  }
  async call(canisterId, options, identity) {
    ...
    const ecid = options.effectiveCanisterId
      ...
      : canister;
    const sender =
      principal_1.Principal.from(id.getPrincipal()) || principal_1.Principal.anonymous();
    let ingress_expiry = new transforms_js_1.Expiry(DEFAULT_INGRESS_EXPIRY_DELTA_IN_MSECS);
    // If the value is off by more than 30 seconds, reconcile system time with the network
    if (Math.abs(this._timeDiffMsecs) > 1000 * 30) {
      ingress_expiry = new transforms_js_1.Expiry(
        DEFAULT_INGRESS_EXPIRY_DELTA_IN_MSECS + this._timeDiffMsecs,
      );
    }
    const submit = {
      request_type: types_js_1.SubmitRequestType.Call,
    ...
    const request = this._requestAndRetry(() =>
      this._fetch(
        ...
        Object.assign(
          Object.assign(Object.assign({}, this._callOptions), transformedRequest.request),
          { body },
        ),
      ),
    );
    const [response, requestId] = await Promise.all([
      request,
      (0, request_id_js_1.requestIdOf)(submit),
    ]);
    return {
      requestId,
      response: {
  ...
  async query(canisterId, fields, identity) {
    ...
    const canister =
      typeof canisterId === 'string' ? principal_1.Principal.fromText(canisterId) : canisterId;
    const sender =
      (id === null || id === void 0 ? void 0 : id.getPrincipal()) ||
      principal_1.Principal.anonymous();
    const request = {
      request_type: 'query' /* ReadRequestType.Query */,
      canister_id: principal_1.Principal.from(canister),
      method_name: fields.methodName,
      arg: fields.arg,
      sender: principal_1.Principal.from(sender),
      ingress_expiry: new transforms_js_1.Expiry(DEFAULT_INGRESS_EXPIRY_DELTA_IN_MSECS),
    };
    ...
    // Apply transform for identity.
    transformedRequest = await (id === null || id === void 0
      ? void 0
      : id.transformRequest(transformedRequest));
    const body = cbor.encode(transformedRequest.body);
    const response = await this._requestAndRetry(() =>
      this._fetch(
        '' + new URL(`/api/v2/canister/${canister.toText()}/query`, this._host),
        Object.assign(
          Object.assign(Object.assign({}, this._fetchOptions), transformedRequest.request),
          { body },
        ),
      ),
    );
  ...
  async readState(
    ...
    const transformedRequest =
      request !== null && request !== void 0
        ? request
        : await this.createReadStateRequest(fields, identity);
    const body = cbor.encode(transformedRequest.body);
  ...
  async syncTime(canisterId) {
    ...
    try {
      ...
      const status = await CanisterStatus.request({
        ...
        canisterId:
          canisterId !== null && canisterId !== void 0
            ? canisterId
            : principal_1.Principal.from('ryjl3-tyaaa-aaaaa-aaaba-cai'),
        agent: this,
        paths: ['time'],
      });
  ...
  async status() {
    ...
    const response = await this._requestAndRetry(() =>
      this._fetch(
        ...
        Object.assign({ headers }, this._fetchOptions),
      ),
    );
    return cbor.decode(await response.arrayBuffer());
  }
