...
import { readIntLE, readUIntLE, writeIntLE, writeUIntLE, powTable } from './utils/leb128';
...
export class FixedIntClass extends PrimitiveType<bigint | number> {
  ...
  public covariant(x: any): x is bigint {
    const min = powTable[this._bits / 8] * BigInt(-1);
    const max = powTable[this._bits / 8] - BigInt(1);
    if (typeof x === 'bigint') {
      return x >= min && x <= max;
...
export class FixedNatClass extends PrimitiveType<bigint | number> {
  ...
  public covariant(x: any): x is bigint {
    const max = powTable[this._bits / 8] * BigInt(2);
