...
export default function OAuthRedirectUriRoute(props: {
  ...
  const parsedBearerToken =
    ...
    icid.response.parseBearerToken(icAuthenticationResponse.accessToken);
  ...
  return (
    <>
      ...
      <p>
        (#todo) This RP page should handle the oauth AuthorizationResponse, create/store the session
        credential, and redirect to the final destination
      </p>
      <dl>
        <dt>AuthenticationResponse</dt>
        <dd>
          <pre>{JSON.stringify(icAuthenticationResponse, null, 2)}</pre>
        </dd>
        <dt>Parsed Bearer Token</dt>
        <dd>
          <details>
            <pre>{JSON.stringify(parsedBearerToken, null, 2)}</pre>
          </details>
        </dd>
        <dt>delegationIdentity</dt>
        <dd>
          {delegationIdentity ? (
            <>
              <dl>
                <dt>publicKey (der)</dt>
                <dd>{hexEncodeUintArray(delegationIdentity.getPublicKey().toDer())}</dd>
                <dt>delegation</dt>
                <dd>
                  <details>
                    <pre>{JSON.stringify(delegationIdentity.getDelegation(), null, 2)}</pre>
                  </details>
                </dd>
              </dl>
            </>
