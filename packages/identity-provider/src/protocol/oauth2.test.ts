...
describe('oauth2', () => {
  ...
  it('can parse AuthorizationRequest fromQueryString', async () => {
    const authorizationRequest = fromQueryString(
      new URLSearchParams(
        '?scope=a%20b&response_type=token&client_id=s6BhdRkqt3&state=xyz&redirect_uri=https%3A%2F%2Fclient%2Eexample%2Ecom%2Fcb&login_hint=xyz',
      ),
    );
    ...
    expect(authorizationRequest.scope).toStrictEqual('a b');
